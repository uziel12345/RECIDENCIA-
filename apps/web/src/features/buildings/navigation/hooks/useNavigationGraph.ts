...
import { getNavigationEdgeWeight } from "@ito-map/shared";

// ── Types ─────────────────────────────────────────────────────────────────────

type AdjEdge = { nodeId: string; weight: number };
...
function getObstacleRadius(building: Building): number {
...
function buildBuildingObstacles(buildings: Building[]): BuildingObstacle[] {
  return buildings
...
function h(a: NavigationNode, b: NavigationNode): number {
  const dx = a.x - b.x;
  const dz = a.z - b.z;
...
function snapToNearest(x: number, z: number, nodes: NavigationNode[]): NavigationNode | null {
...
export function useNavigationGraph() {
  ...
  useEffect(() => {
    ...
    Promise.all([getNavigationNodes(), getNavigationEdges(), getBuildings()])
      .then(([nodes, edges, buildings]) => {
        if (!mounted) return;
  ...
  const findPath = useCallback(
    (from: THREE.Vector3, to: THREE.Vector3): THREE.Vector3[] => {
      ...
      // El último punto es la coordenada exacta de la entrada del edificio
      // El primer punto es el nodo de snap (en el pasillo), NO la posición GPS cruda
      // La posición GPS la muestra UserLocationMarker por separado
