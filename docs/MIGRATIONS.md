...
| `migration-building-model-coordinates-20260804.sql` | Sustituye X/Z heredados del modelo anterior por los centros geométricos de los nodos del `campus.glb` actual; sincroniza puntos de calibración y desactiva perfiles affine obsoletos. | Aplicado y verificado en producción; respaldo `mapa-ito-20260804T214531Z.tar.gz` | 2026-08-04 |
| `migration-campus-discovery-20260805.sql` | Agrega cargos públicos, calles, consultas rápidas, alias y estado de validación de servicios. Reversión en `rollback-campus-discovery-20260805.sql`; datos en `seed-campus-discovery-20260805.sql`. | Aplicado y verificado en producción (confirmado 2026-08-12) y en BD local (`mapa_ito`) el 2026-08-06 — incorporado en el esquema actual | 2026-08-05 |
...
| `migration-vincular-tramites-edificios-20260812.sql` | Vincula 4 trámites de `seed-campus-discovery-20260805.sql` a su edificio real vía `building_procedures` (confirmado por el usuario); crea los departamentos "Servicio Social", "Coordinación" y "Servicios Escolares" bajo DIR; marca esos 4 trámites como `validation_status='confirmed'`. | Aplicado y verificado en producción; respaldo `mapa-ito-20260812T160958Z.tar.gz` | 2026-08-12 |
| `migration-tramites-department-id-20260812.sql` | Completa `procedures.department_id` para los 3 trámites de la migración anterior que sí tienen un departamento propio en su edificio (deja fuera el de Gimnasio, que no tiene departamento registrado) — permite que la UI anide cada trámite dentro de la tarjeta de su departamento. | Aplicado y verificado en producción; respaldo `mapa-ito-20260812T172908Z.tar.gz` | 2026-08-12 |
